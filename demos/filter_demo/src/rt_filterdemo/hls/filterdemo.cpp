...
#define macro_prefetch_rows {\
	for(int i = 0; i < PREFETCH_ROWS; i++) {\
		BASETYPE _offset = ((ptr_i + i * img_w) & MASK);\
		BASETYPE _len = (img_w + _offset + BYTES)&(~MASK);\
		MEM_READ(((ptr_i + i * img_w)&(~MASK)), &_in[0], _len);\
...
#define macro_read_row {\
	...
	BASETYPE _offset = ((ptr_i + (PREFETCH_ROWS + (ptr_limit)) * img_w) & MASK);\
	BASETYPE _len = (img_w + _offset + BYTES)&(~MASK);\
	MEM_READ(((ptr_i + (PREFETCH_ROWS + (ptr_limit)) * img_w)&(~MASK)), &_in[0], _len);\
	for(int ii = 0; ii < CC_W; ii++) {\
		uint8_t _byte_in_dword = (uint8_t)((_offset + ii) % BYTES);\
		uint16_t _dword_ptr = (uint16_t)((_offset + ii) / BYTES);\
		BASETYPE _dword = _in[_dword_ptr];\
...
#define macro_write_row {\
	MEM_WRITE(&_out[0], (ptr_o + row*CC_W), CC_W);\
}
