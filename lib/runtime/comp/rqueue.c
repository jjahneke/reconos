...
void rq_send(rqueue *rq, uint64_t *msg, size_t size)
{
	/* XXX: Can we also avoid allocation + copy?! ---DB */
	uint64_t *clone = malloc((size + 1) * sizeof(uint64_t));
	if (!clone)
		panic("rq_send malloc failed\n");

	clone[0] = (uint64_t) size;
	__builtin_memcpy(&clone[1], msg, size);

	mbox_put((struct mbox *) rq, (uint64_t) clone);
}

ssize_t rq_receive(rqueue *rq, uint64_t *msg, size_t size)
{
	uint64_t *clone;
	ssize_t __size;

	clone = (uint64_t *) mbox_get((struct mbox *) rq);
	__size = clone[0];
