...
puts "\[DEBUG\] After set ip_repo"
puts "ip_repo is at $ip_repo"
set repo_path [get_property ip_repo_paths [current_project]]
foreach n [get_ipdefs -filter REPOSITORY==$repo_path] { puts "$n" }

import_pcore $ip_repo reconos_v3_01_a ""; # ReconOS Lib has to be imported first, so other IP can use it

puts "\[DEBUG\] After include reconos into ip_repo"
set repo_path [get_property ip_repo_paths [current_project]]
foreach n [get_ipdefs -filter REPOSITORY==$repo_path] { puts "$n" }
#close_project
